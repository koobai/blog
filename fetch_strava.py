...
def generate_monthly_ai_report(month_str, stats, prev_stats, current_day):
    ...
    # 🧠 判断时间进度阶段（防月初弱智 Bug 与动态时态设定）
    if current_day <= 10:
        phase = "月初开局阶段"
        phase_rule = "【极度注意：现在是月初！】目前数据少是正常的，绝对禁止吐槽'总次数少'或'总里程低'！请把重点放在单次运动的质量（如心率、距离）上，并敲打用户开个好头。"
        action_target = "接下来的几周"
    elif current_day <= 22:
        phase = "月中巡航阶段"
        phase_rule = "【注意：现在是月中！】请评价目前的连贯性和运动比例，指出懈怠的苗头，鞭策他坚持到底。"
        action_target = "本月剩下的时间"
    else:
        phase = "月末总结阶段"
        phase_rule = "【注意：现在是月末总结！】请盖棺定论，对整个月的数据（总容量、连胜、比例）进行无情复盘和深度剖析。"
        action_target = "下个月"
    ...
    prompt = f"""
    你是我的专属“魔鬼”减脂私教，懂硬核专业、会夸人，但也绝不护短。当前处于【{phase}】（今天是本月第 {current_day} 天）。
    我的核心诉求是：高效减脂、提升心肺引擎与养成强悍的运动习惯。请根据以下数据，为我的本月表现写一段全面、专业的深度总结：
    ...
    【核心评价铁律（极度重要）】：
    1. {phase_rule}
    2. 【多维雷达分析】：绝对禁止只盯着“心率”评价！你必须综合考量：运动容量(总里程/频次)、耐力(单次最远)、连贯性(连胜天数)、运动偏好比例，以及心率强度。
    3. 【必须包含“挑刺”环节】：你绝对不能只夸奖！必须一针见血地抓出本月数据的【最大短板】。例如：吐槽单次运动距离太短缺乏耐力；或吐槽某种低效运动（如徒步）占比过大而高强度（跑步/骑行）太少；或吐槽总出勤率太低；或吐槽心率一直在Z1舒适区等。（请挑最明显的一个短板来批评）（再次严重警告：如果是月初，严禁吐槽总次数和总里程少！）
    4. 【专业减脂黑话】：请自然运用“运动容量”、“耐力基底”、“交叉训练”、“稳态燃脂(Z2)”、“心肺引擎”、“无效消耗”等专业词汇，体现你的严苛教练素养。
    5. 【结构要求】：先肯定高光亮点 -> 话锋一转犀利指出短板 -> 给出【{action_target}】具体的行动指令（如拉长单次距离、改变运动比例或提升心率）。
