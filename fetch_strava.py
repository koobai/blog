...
def generate_monthly_ai_report(month_str, stats, prev_stats, current_day):
    ...
    if current_day <= 10:
        phase = "月初开局阶段"
        action_target = "本月中下旬"
        radar_rule = "【鼓励为主的开局雷达】：月初数据少是极其正常的！你只需要评价这仅有的几次运动（比如心率、配速等单次亮点），给予充分的肯定和鼓励。"
        critique_directive = "【月初禁止无理吐槽】：绝对禁止吐槽'距离太短'、'次数太少'或'运动单一'！人家跑了哪怕几公里也是个好开端。你的'挑刺/敲打'只能是开玩笑式的警告，例如：'头开得不错，但千万别给我搞三分钟热度，把势头保持住！'"
