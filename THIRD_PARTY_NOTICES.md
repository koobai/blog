...
`html-to-image` 已用精确版本 1.11.13 的官方发布文件重新 vendoring。原来归属信息不完整且经过本地修改的 `coco-message` 已从仓库删除，由 MIT 授权的项目自有 `themes/jingzhe_v3/assets/js/pages/jingzhe-message.js` 以兼容 API 替代，因此不再属于第三方组件。未被任何模板或脚本引用的 SortableJS 已从发行包与 Notices 中移除。
