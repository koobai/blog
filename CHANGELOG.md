...
- 主题样式从已弃用的 LibSass/SCSS 迁移为原生 CSS 与 Hugo `css.Build`，不增加 Dart Sass、Node 或 npm 依赖。
- 网页写作模板改为独立共享 Core 与页面模块，保留原地址、草稿 Key、发布路径和 Worker 协议；Front Matter、文件路径、图片上传与未保存编辑获得更明确的失败保护。
- 自有页面 JavaScript 改由 Hugo Pipes 在 Production 自动压缩并生成内容指纹，开发服务器继续提供可读源码；写作管理页默认禁止搜索引擎索引。
- 唠叨推荐改为稳定的相关内容与近期内容，并复用缓存后的卡片渲染。
- 点赞前端改为即时反馈，服务端成功后确认、重复点赞时校准、请求失败时回滚；Likes Worker 原子写入访客去重记录和页面计数。
- 豆瓣同步读取完整分页，合并新增和已有记录更新，写入采用原子替换；同步失败时非零退出且保留原数据。
- 运动展示名称、成就字段和地标选择规则统一在处理阶段与单一数据源中生成，模板和浏览器不再重复业务判断。
- 运动前端按数据模型、日历 UI、隐私路线、Mapbox、海报与控制器拆分，并继续合并为一个指纹化脚本；`window.KoobaiRun` 兼容接口保持不变。
- 运动处理与 AI 月报按稳定职责拆入 `jingzhe/`，根脚本和 Actions 命令保持兼容，处理后 JSON 与隐私边界不变。
- 原生 App 的 `pending` 路线状态纳入数据契约和跨项目回归；除 `available` 外的状态均不得携带公开轨迹。
- 未变化的历史月报不再刷新 `last_update`，避免工作流产生无意义的数据提交；迟到数据修正和终稿冻结规则不变。
- GitHub Actions 的基础 Action 版本保持一致，Hugo 工作流继续跟随 `latest`。
- 主要同步、处理和部署工作流增加运行摘要，触发方式、提交约定和 Secrets 保持不变。
...
- 删除未被页面或脚本引用的 SortableJS 及对应许可证副本。
