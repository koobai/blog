...
2. 保留主题中的 `layouts/exercise.html`、`layouts/_partials/exercise-food-icons.html`、`assets/css/exercise.css` 与 `assets/js/exercise/` 模块。
...
首次迁移使用 `snapshot`，日常新增或修正使用 `delta`。交付前至少验证：
...
满足以上七项，才算“动起来”页面、同步入口和自动发布全部部署完成；只看到 Worker 在线或页面能打开都不算完整验收。
